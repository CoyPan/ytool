...
var ytool = {
    ...
    /**
     *  touch the {dom} for {duration}ms and then exec the callback {cb}
     * 
     *  @param {HTMLElement} dom
     *  @param {Funciton} cb
     *  @param {number} duration
     *  @param {isPreventDefault} boolean
     */
    longTouch: function (dom, cb, duration, isPreventDefault) {
        duration = duration || 1000;
        isPreventDefault = isPreventDefault !== undefined ? isPreventDefault : false;
        if (!dom) {
            throw 'longTouch error';
        }
        var timer = null;
        var flag = false;
